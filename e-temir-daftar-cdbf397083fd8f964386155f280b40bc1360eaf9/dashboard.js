...
/**
 * Foydalanuvchini qo'shish kerakmi yoki yo'qmi tekshiradi
 * @param {Array} users - Barcha foydalanuvchilar ro'yxati [{accountId, name}]
 * @param {string} accountId - Joriy account ID
 * @param {string} name - Qo'shilayotgan ism
 * @returns {boolean} - true bo'lsa qo'shilsin, false bo'lsa qo'shilmasin
 */
function shouldAddUser(users, accountId, name) {
    // Shu accountda shu ism bormi?
    const existsInThisAccount = users.some(
        user => user.accountId === accountId && user.name === name
    );
    if (existsInThisAccount) return false;

    // Boshqa accountlarda shu ism bormi?
    const existsInOtherAccounts = users.some(
        user => user.accountId !== accountId && user.name === name
    );
    // Agar boshqa accountda bo'lsa yoki umuman bo'lmasa, qo'shilsin
    return existsInOtherAccounts || !users.some(user => user.name === name);
}

module.exports = { shouldAddUser };